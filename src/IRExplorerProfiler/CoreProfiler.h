...
inline void Log(const std::string format, ...)
{
	...
	va_list args;
	va_start(args, format);
	_vsnprintf_s<TRACE_BUFFER_SIZE>(buffer, TRACE_BUFFER_SIZE - 2, format.c_str(), args); // Truncates if buffer is not big enough.
	va_end(args);
...
private:
	...
	bool SendLoadedFunctionCode(uint64_t funcId, uint64_t address, uint32_t rejitId, uint32_t codeSize, char* codeByes);
	bool SendCallTargets(uint64_t funcId, uint32_t rejitId, uint32_t codeSize, char* codeByes);
	...
	void CollectCallTargets(uint64_t funcId, uint32_t rejitId, void* buffer, size_t size)
	{
		auto dis = DisasmType();
			...
			if (instr->detail)
			{
				for (int i = 0; i < instr->detail->x86.op_count; i++)
				{
					if (instr->detail->x86.operands[i].type == X86_OP_IMM)
					{
						...
						{
							Log("IRX: Already sent target %lld\n", targetAddr);
						}
					}
				}
			}
	...
	void CollectCallTargetsArm64(uint64_t funcId, uint32_t rejitId, void* buffer, size_t size)
	{
