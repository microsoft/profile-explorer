...
namespace Core.Analysis {
    ...
    public sealed class ReferenceFinder {
        ...
        public IRElement FindEquivalentValue(IRElement element, bool onlySSA = false) {
            ...
            return FindBestMatchingOperand(EnumerateAllOperands(), op);
        }
        ...
        private IRElement FindBestMatchingOperand(IEnumerable<IRElement> candidates,
            ...
            foreach (var useElement in candidates) {
                ...
                if (useOp.Role == op.Role &&
                    IsSameBlock(useOp.ParentBlock, opBlock)) {
        ...
        private bool IsSameBlock(BlockIR firstBlock, BlockIR secondBlock) {
            if (firstBlock == secondBlock ||
                firstBlock.Number == secondBlock.Number) {
                return true;
            }

            if (firstBlock.HasLabel && secondBlock.HasLabel) {
                return MemoryExtensions.Equals(firstBlock.Label.Name.Span,
                                               secondBlock.Label.Name.Span,
        ...
        public OperandIR FindOperandWithSSADefinitionID(int defId) {
            ...
            if(ssaDefTagMap_.TryGetValue(defId, out var tag)) {
                return tag.DefinitionOperand;
            }
        ...
        public List<Reference> FindAllDefUsesOrReferences(IRElement element) {
            if (element is OperandIR op) {
                ...
                if (ssaDef != null) {
                    ...
                    list.Add(new Reference(ssaDef, ReferenceKind.SSA));
                    FindSSAUses(op, list);
                    return list;
                }
            }
        ...
        public List<Reference> FindAllReferences(IRElement element, bool includeSSAUses = true,
                                                 Func<IRElement, ReferenceKind, bool> filterAction = null) {
                ...
                foreach (var tuple in block.Tuples) {
                    ...
                    foreach (var sourceOp in instr.Sources) {
                        if (IsSameOperand(sourceOp, op)) {
                            var refKind = sourceOp.IsAddress ? ReferenceKind.Address :
                                                                 ReferenceKind.Load;
                            if (filterAction != null) {
        ...
        public List<Reference> FindAllStores(IRElement element) {
            return FindAllReferences(element, false, (element, kind) => kind == ReferenceKind.Store);
        }

        public List<Reference> FindAllLoads(IRElement element) {
            return FindAllReferences(element, false, (element, kind) => kind == ReferenceKind.Load);
        }
        ...
        private void FindSSAUses(OperandIR op, List<Reference> list) {
            foreach (var use in EnumerateSSAUses(op)) {
                list.Add(new Reference(use, ReferenceKind.SSA));
            }
        }

        private IEnumerable<IRElement> EnumerateSSAUses(OperandIR op) {
        ...
        private bool IsSameOperand(OperandIR op, OperandIR searchedOp, bool checkType = false) {
            ...
            if (op.Kind == OperandKind.Variable ||
                ...
                if (!MemoryExtensions.Equals(op.NameValue.Span, searchedOp.NameValue.Span,
                                               StringComparison.Ordinal)) {
                    return false;
                }
