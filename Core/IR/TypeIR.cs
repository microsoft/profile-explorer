...
namespace Core.IR {
    ...
    public sealed class TypeIR {
        ...
        private TypeIR(TypeKind kind, int size,
        ...
        public static TypeIR GetInt(int size) {
            switch (size) {
                case 1: return signedIntTypes_[0];
                case 2: return signedIntTypes_[1];
                case 4: return signedIntTypes_[2];
                case 8: return signedIntTypes_[3];
                default: {
                    return GetType(TypeKind.Integer, size, TypeFlags.SignedInt);
                }
            }
        }
        ...
        public static TypeIR GetType(TypeKind kind, int size,
                                     TypeFlags flags = TypeFlags.None) {
            TypeIR type = new TypeIR(kind, size, flags);
            return uniqueTypes_.GetOrAdd(type, type);
        }

        public static TypeIR GetUInt(int size) {
            switch (size) {
                case 1: return unsignedIntTypes_[0];
                case 2: return unsignedIntTypes_[1];
                case 4: return unsignedIntTypes_[2];
                case 8: return unsignedIntTypes_[3];
                default: {
                    return GetType(TypeKind.Integer, size, TypeFlags.UnsignedInt);
                }
            }
        }
        ...
        public bool IsInt => Kind == TypeKind.Integer &&
                             Flags != TypeFlags.UnsignedInt;
        public bool IsInteger => Kind == TypeKind.Integer;
        public bool IsMultibyte => Kind == TypeKind.Multibyte;
        public bool IsUInt => Kind == TypeKind.Integer &&
                              Flags == TypeFlags.UnsignedInt;
        ...
        public override string ToString() {
            switch (Kind) {
                case TypeKind.Integer: {
                    if (IsUInt) {
                        return string.Format("uint{0}", Size * 8);
                    }
                    else return string.Format("int{0}", Size * 8);
                }
                case TypeKind.Float:
                return string.Format("float{0}", Size * 8);
                case TypeKind.Vector:
                return string.Format("vector{0}", Size * 8);
                case TypeKind.Multibyte:
                return string.Format("mb{0}", Size);
                case TypeKind.Array:
                return "<TODO:array>";
