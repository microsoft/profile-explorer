...
namespace Client {
    ...
    public partial class SectionPanelPair : ToolPanelControl {
        ...
        public ICompilerIRInfoProvider CompilerInfo {
        ...
        public bool HasAnnotatedSections => MainPanel.HasAnnotatedSections ||
                                            DiffPanel.HasAnnotatedSections;
        ...
        public IRTextSection FindDiffDocumentSection(IRTextSection section) {
            ...
            var sectionIndex = panel.Sections.FindIndex((item) => item.Section == section);
        ...
        (List<IRTextSectionExtension>, List<IRTextSectionExtension>)
            ComputeSectionNameListDiff(List<IRTextSectionExtension> baseList,
                                       List<IRTextSectionExtension> diffList) {
            ...
            while (!(x == 0 && y == 0)) {
                ...
                if (y > 0 && (x == 0 || m[x, y - 1] >= m[x - 1, y])) {
                    ...
                    var diffSection = diffList[y - 1];
                    newDiffList.Add(new IRTextSectionExtension(diffSection.Section, DiffKind.Insertion, diffSection.Name));
                    ...
                    var baseSection = baseList[x - 1];
                    newBaseList.Add(new IRTextSectionExtension(baseSection.Section, DiffKind.Deletion, baseSection.Name));
        ...
        async Task<List<HasDiffResult>>
            ComputeSectionIRDiffs(List<IRTextSectionExtension> baseSections,
                                  List<IRTextSectionExtension> diffSections) {
            ...
            var results = await DocumentDiff.ComputeSectionDiffs(comparedSections,
                                                                 baseLoader, diffLoader);
            return results;
        }
