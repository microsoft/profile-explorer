...
namespace Client {
    ...
    public class ReferenceInfo {
        ...
        public string Kind {
            get {
                switch (Info.Kind) {
                    case ReferenceKind.Address: return "Address";
                    case ReferenceKind.Load: return "Load";
                    case ReferenceKind.Store: return "Store";
                    case ReferenceKind.SSA: return "SSA use";
                }
                return "";
            }
        }
    ...
    public class ReferencePanelState {
        ...
        public IRElement Element { get => elementRef_; set => elementRef_ = value; }
    ...
    public partial class ReferencesPanel : ToolPanelControl {
        ...
        private (TextBlock, string) CreatePreviewTextBlock(OperandIR operand, Reference reference) {
            var text = FindPreviewText(reference);
            var symbolName = ReferenceFinder.GetSymbolName(operand);
            int index = 0;
            ...
            while (index < text.Length) {
                var symbolIndex = text.IndexOf(symbolName, index);
        ...
        private string FindPreviewText(Reference reference) {
            ...
            var text = instr.GetText(document_.Text).ToString();
            int start = 0;
            int length = text.Length;

            if (instr.Destinations.Count > 0) {
                var firstDest = instr.Destinations[0];
                start = firstDest.TextLocation.Offset -
        ...
        public List<Reference> FindAllReferences(IRElement element, bool pinElement = true) {
            var operand = element as OperandIR;
            ...
            ReferenceFinder refFinder = new ReferenceFinder(document_.Function);
        ...
        private void UpdateReferenceListView(OperandIR operand, List<Reference> operandRefs) {
            ...
            operandRefs.Sort((a, b) => {
                return a.Element.TextLocation.Offset - b.Element.TextLocation.Offset;
            });

            foreach (var reference in operandRefs) {
                (var preview, var previewText) = CreatePreviewTextBlock(operand, reference);
                referenceList_.Add(new ReferenceInfo(index, reference, preview, previewText));
                index++;

                switch (reference.Kind) {
                    case ReferenceKind.Address: referenceSummary_.AddressCount++; break;
                    case ReferenceKind.Load: referenceSummary_.LoadCount++; break;
                    case ReferenceKind.Store: referenceSummary_.StoreCount++; break;
                    case ReferenceKind.SSA: referenceSummary_.SSACount++; break;
                }
            }

            referenceListView_ = new ListCollectionView(referenceList_);
            referenceListView_.Filter = new Predicate<object>(FilterReferenceList);
        ...
        public IRElement Element {
            ...
            set {
                ...
                if (element_ != value) {
                    ...
                    if (FindSSAUses(element_, pinElement: false).Count == 0) {
                        FindAllReferences(element_, pinElement: false);
                    }
                }
            }
        }
        ...
        private void FilterComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e) {
            ...
            if (kindString == "All") {
                ...
                switch (kindString) {
                    case "SSA": filterKind_ = ReferenceKind.SSA; break;
                    case "Load": filterKind_ = ReferenceKind.Load; break;
                    case "Store": filterKind_ = ReferenceKind.Store; break;
                    case "Address": filterKind_ = ReferenceKind.Address; break;
                }
            }
        ...
        private void MarkReferenceExecuted(object sender, ExecutedRoutedEventArgs e) {
            ...
            if (refInfo != null) {
                var color = (e.Parameter as ColorEventArgs).SelectedColor;
        ...
        public override void OnDocumentSectionUnloaded(IRTextSection section, IRDocument document) {
            ...
            var data = StateSerializer.Serialize<ReferencePanelState>(state, document.Function);
