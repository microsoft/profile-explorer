...
namespace Client {
    ...
    public class Bookmark {
        ...
        public Bookmark(int index, IRElement element,
                        string text, HighlightingStyle style) {
            Index = index;
            Element = element;
            Text = text;
            Style = style;
        ...
        public Brush StyleBackColor => Style != null ? Style.BackColor : null;
    ...
    public class BookmarkManager {
        ...
        public byte[] SaveState(FunctionIR function) {
            ...
            bookmarkState.elementBookmarkMap_ = elementBookmarkMap_?
                .ToList<IRElement, IRElementReference, Bookmark>();
            bookmarkState.nextIndex_ = nextIndex_;
            bookmarkState.selectedIndex_ = selectedIndex_;
            return StateSerializer.Serialize<BookmarkManagerState>(bookmarkState, function);
        }
        ...
        public void LoadState(byte[] data, FunctionIR function) {
            ...
            elementBookmarkMap_ = bookmarkState.elementBookmarkMap_?
                .ToDictionary<IRElementReference, IRElement, Bookmark>() ??
        ...
        public Bookmark FindBookmark(IRElement element) {
            if (elementBookmarkMap_.TryGetValue(element, out Bookmark value)) {
                return value;
            }
        ...
        public Bookmark RemoveBookmark(IRElement element) {
            if (elementBookmarkMap_.TryGetValue(element, out Bookmark bookmark)) {
