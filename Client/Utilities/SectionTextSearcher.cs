...
namespace Client {
    ...
    public class SectionTextSearcher {
        ...
        public async Task<List<SectionSearchResult>>
            SearchAsync(string searchedText, TextSearchKind searchKind, List<IRTextSection> sections) {
            ...
            for (int i = 0; i < sections.Count; i++) {
                ...
                tasks[i] = Task.Run<SectionSearchResult>(() => {
                    return SearchSection(searchedText, searchKind, section);
                });
            }
        ...
        public async Task<SectionSearchResult>
            SearchSectionAsync(string searchedText, TextSearchKind searchKind, IRTextSection section) {
        ...
        public SectionSearchResult SearchSection(string searchedText, TextSearchKind searchKind,
            ...
            var text = sectionLoader_.LoadSectionText(section);
            return SearchSection(text, searchedText, searchKind, section);
        }

        public SectionSearchResult SearchSection(string text, string searchedText,
                                                 TextSearchKind searchKind, IRTextSection section) {
