...
namespace Client {
    ...
    public struct TextSearchResult {
        ...
        public override bool Equals(object obj) {
            return obj is TextSearchResult result &&
    ...
    public class TextSearcher {
        ...
        private static (int, int) IndexOf(string text, string searchedText, int startOffset = 0,
            ...
            if (string.IsNullOrEmpty(text) ||
                string.IsNullOrEmpty(searchedText)) {
                return (-1, 0);
            }
        ...
        public static (int, int) IndexOf(string text, string searchedText, int startOffset = 0,
                                         TextSearchKind searchKind = TextSearchKind.Default) {
            return IndexOf(text, searchedText, startOffset, searchKind, null);
        }

        public static List<TextSearchResult> AllIndexesOf(string text, string searchedText, int startOffset = 0,
                                                  TextSearchKind searchKind = TextSearchKind.Default) {
            if (string.IsNullOrEmpty(text) ||
                string.IsNullOrEmpty(searchedText)) {
            ...
            Regex regex = searchKind.HasFlag(TextSearchKind.Regex) ?
                          CreateRegex(searchedText, searchKind) : null;
            var offsetList = new List<TextSearchResult>();
            var (offset, length) = IndexOf(text, searchedText, startOffset, searchKind, regex);
        ...
        public static bool Contains(string text, string searchedText,
                                    TextSearchKind searchKind = TextSearchKind.Default) {
            var (offset, length) = IndexOf(text, searchedText, 0, searchKind, null);
            return offset != -1;
        }
    }
}
